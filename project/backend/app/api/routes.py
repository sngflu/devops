...
active_sessions_total = Gauge("active_sessions_total", "Number of active user sessions")

# Метрики для работы с видео
video_views_total = Counter("video_views_total", "Total number of video views")
...
db_operation_latency = Histogram(
    ...
    [
        "operation",
        "table",
    ],  # operation: get, create, update, delete; table: users, videos, etc.
)
...
api_response_size_bytes = Histogram(
    "api_response_size_bytes", "Size of API responses in bytes", ["endpoint"]
)
...
videos_per_user = Gauge("videos_per_user", "Average number of videos per user")
...
def register():
    try:
        ...
        db_operation_latency.labels(operation="get", table="users").observe(
            time.time() - db_start
        )
        ...
        db_operation_latency.labels(operation="create", table="users").observe(
            time.time() - db_start
        )
        ...
        api_response_size_bytes.labels(endpoint="/register").observe(
...
def login():
    try:
        ...
        if user and check_password_hash(user["password_hash"], password):
            ...
            # Измеряем размер ответа
            response = jsonify({"token": token})
            api_response_size_bytes.labels(endpoint="/login").observe(
                len(response.get_data(as_text=True))
            )
            return response
...
def processing():
    ...
    if (
        ...
        return (
            jsonify(
                {
                    "error": "Недопустимый формат файла. Разрешены только видеофайлы (.mp4, .avi, .mov, .mkv)"
                }
            ),
            400,
        )
    ...
    try:
        ...
        logger.info(
            f"Начало обработки видео: {file.filename}, порог уверенности: {confidence_threshold}"
        )
        video_filename, frame_objects, fps, has_weapon_or_knife, log_filename = (
            video_processing.process_video(temp_path, confidence_threshold, username)
        )
        ...
        if not video_filename or not isinstance(frame_objects, list) or not fps:
            ...
            raise ValueError(
                "Не удалось корректно обработать видео. Проверьте формат файла."
            )

        logger.info(
            f"Обработка видео завершена: {video_filename}, кадров: {len(frame_objects)}, fps: {fps}"
        )
        ...
        if user_id:
            ...
            success, error1 = db_manager.save_detection_results(
                video_id, log_filename, frame_objects, has_weapon_or_knife
            )
            ...
            if error1:
                logger.error(
                    f"Ошибка при сохранении результатов обнаружения в БД: {error1}"
                )
            else:
        ...
        return (
            jsonify(
                {
                    "error": "Произошла ошибка при обработке видео. Пожалуйста, попробуйте снова или используйте другой файл."
                }
            ),
            500,
        )
    ...
    try:
        ...
        if not filename.startswith(f"{username}_"):
            api_errors_total.labels(
                endpoint="/video/<filename>", error_type="unauthorized"
            ).inc()
        ...
        try:
            if user_id:
                ...
                db_operation_latency.labels(operation="get", table="videos").observe(
                    time.time() - db_start
                )
            ...
            minio_operation_latency.labels(operation_type="get_presigned_url").observe(
                time.time() - minio_start
            )
            ...
            if video_url:
                ...
                video_operations_total.labels(
                    operation_type="view", status="success"
                ).inc()
                ...
                response = (
                    ...
                    else jsonify({"url": video_url})
                )
                if not request.args.get("direct"):  # Измеряем только для JSON ответов
                    api_response_size_bytes.labels(
                        endpoint="/video/<filename>"
                    ).observe(len(response.get_data(as_text=True)))

                # Общее время обработки запроса
                api_request_duration = time.time() - start_time
                logger.debug(
                    f"Время обработки запроса video/{filename}: {api_request_duration:.4f} сек"
                )

                return response, 200
            else:
                logger.error(
                    f"Не удалось получить временную ссылку из MinIO для {filename}"
                )
                video_operations_total.labels(
                    operation_type="view", status="error_not_found"
                ).inc()
                api_errors_total.labels(
                    endpoint="/video/<filename>", error_type="minio_error"
                ).inc()
            ...
            logger.error(traceback.format_exc())
            video_operations_total.labels(
                operation_type="view", status="error_exception"
            ).inc()
            api_errors_total.labels(
                endpoint="/video/<filename>", error_type="exception"
            ).inc()
            return jsonify({"error": str(e)}), 500
    except Exception as e:
        api_errors_total.labels(
            endpoint="/video/<filename>", error_type="auth_exception"
        ).inc()
        raise e
...
def get_videos():
    ...
    try:
        ...
        try:
            ...
            minio_operation_latency.labels(operation_type="list_videos").observe(
                time.time() - minio_start
            )
            ...
            api_response_size_bytes.labels(endpoint="/videos").observe(
                len(response.get_data(as_text=True))
            )
            ...
            api_request_duration = time.time() - start_time
            logger.debug(
                f"Время обработки запроса /videos: {api_request_duration:.4f} сек"
            )
            ...
            video_operations_total.labels(
                operation_type="list", status="error_exception"
            ).inc()
    ...
    try:
        if user_id:
            ...
            if video_data:
                ...
                detection_results = db_manager.get_video_detections(
                    video_data["video_id"]
                )
                if detection_results:
                    if storage.object_exists(
                        detection_results["bucket_name"], detection_results["s3_key"]
                    ):
                        logs = storage.get_log_from_bucket(
                            detection_results["bucket_name"],
                            detection_results["s3_key"],
                        )
                        if logs:
                            print(logs)
                            return jsonify(logs)
...
def delete_video_route(filename):
    ...
    try:
        ...
        if not filename.startswith(f"{username}_"):
            video_operations_total.labels(
                operation_type="delete", status="error_unauthorized"
            ).inc()
            api_errors_total.labels(
                endpoint="/videos/<filename>/delete", error_type="unauthorized"
            ).inc()
            return jsonify({"error": "Unauthorized"}), 401
        ...
        try:
            ...
            if user_id:
                ...
                if video_data:
                    ...
                    db_start = time.time()
                    success, result = db_manager.delete_video(
                        video_data["video_id"], user_id
                    )
                    db_operation_latency.labels(
                        operation="delete", table="videos"
                    ).observe(time.time() - db_start)
            ...
            minio_operation_latency.labels(operation_type="delete_objects").observe(
                time.time() - minio_start
            )

            if not success and not deleted_from_db:
                video_operations_total.labels(
                    operation_type="delete", status="error_storage"
                ).inc()
                api_errors_total.labels(
                    endpoint="/videos/<filename>/delete", error_type="storage_error"
                ).inc()
                return jsonify({"error": "Failed to delete video"}), 500

            # Инкрементируем счетчик успешных операций удаления
            video_operations_total.labels(
                operation_type="delete", status="success"
            ).inc()

            # Общее время обработки запроса
            api_request_duration = time.time() - start_time
            logger.debug(
                f"Время обработки запроса DELETE /videos/{filename}: {api_request_duration:.4f} сек"
            )
            ...
            video_operations_total.labels(
                operation_type="delete", status="error_exception"
            ).inc()
            api_errors_total.labels(
                endpoint="/videos/<filename>/delete", error_type="exception"
            ).inc()
            return jsonify({"error": str(e)}), 500
    except Exception as e:
        api_errors_total.labels(
            endpoint="/videos/<filename>/delete", error_type="auth_exception"
        ).inc()
        raise e
...
def update_video(filename):
    ...
    try:
        ...
        if not new_name:
            video_operations_total.labels(
                operation_type="rename", status="error_invalid_input"
            ).inc()
            return jsonify({"error": "New name is required"}), 400

        if not filename.startswith(f"{username}_"):
            video_operations_total.labels(
                operation_type="rename", status="error_unauthorized"
            ).inc()
            api_errors_total.labels(
                endpoint="/videos/<filename>/rename", error_type="unauthorized"
            ).inc()
            return jsonify({"error": "Unauthorized"}), 401
        ...
        try:
            ...
            if len(parts) < 4:
                video_operations_total.labels(
                    operation_type="rename", status="error_invalid_filename"
                ).inc()
            ...
            if user_id:
                ...
                if video_data:
                    ...
                    success, error = db_manager.rename_video(
                        video_data["video_id"], user_id, new_filename
                    )
                    db_operation_latency.labels(
                        operation="update", table="videos"
                    ).observe(time.time() - db_start)

                    if success:
                        updated_in_db = True
                        logger.info(
                            f"Обновлено имя видео в БД: {filename} -> {new_filename}"
                        )
                    else:
                        logger.error(f"Ошибка при обновлении имени видео в БД: {error}")
                        video_operations_total.labels(
                            operation_type="rename", status="error_db"
                        ).inc()
                        api_errors_total.labels(
                            endpoint="/videos/<filename>/rename", error_type="db_error"
                        ).inc()
            ...
            minio_operation_latency.labels(operation_type="rename_object").observe(
                time.time() - minio_start
            )
            ...
            storage.rename_object(
                storage.log_bucket, f"{filename}.json", f"{new_filename}.json"
            )
            minio_operation_latency.labels(operation_type="rename_object").observe(
                time.time() - minio_start
            )
            ...
            video_operations_total.labels(
                operation_type="rename", status="success"
            ).inc()

            # Общее время обработки запроса
            api_request_duration = time.time() - start_time
            logger.debug(
                f"Время обработки запроса PUT /videos/{filename}: {api_request_duration:.4f} сек"
            )

            return jsonify(
                {"message": "Video renamed successfully", "new_filename": new_filename}
            )
        except Exception as e:
            logger.error(f"Ошибка при переименовании видео: {str(e)}")
            video_operations_total.labels(
                operation_type="rename", status="error_exception"
            ).inc()
            api_errors_total.labels(
                endpoint="/videos/<filename>/rename", error_type="exception"
            ).inc()
            return jsonify({"error": str(e)}), 500
    except Exception as e:
        api_errors_total.labels(
            endpoint="/videos/<filename>/rename", error_type="auth_exception"
        ).inc()
        raise e
