...
def app():
    ...
    with patch("app.api.routes.SECRET_KEY", TEST_SECRET_KEY), patch(
        "app.api.routes.DatabaseManager"
    ) as mock_db_manager, patch("app.api.routes.MinioStorage") as mock_minio, patch(
        ...
        app = create_app({"TESTING": True, "SECRET_KEY": TEST_SECRET_KEY})
...
def auth_token():
    ...
    return jwt.encode({"user": "testuser", "user_id": "user123"}, TEST_SECRET_KEY)
...
def mock_db_manager(monkeypatch):
    ...
    # Мокаем методы, которые используются в тестах
    mock.get_user_by_username.return_value = None
    mock.create_user.return_value = (1, None)
    mock.init_database.return_value = None

    monkeypatch.setattr("app.services.database.DatabaseManager", lambda: mock)
    yield
...
def mock_minio_storage():
    ...
    with patch("app.services.minio.minio_client.Minio") as mock_minio_client:
        ...
        mock_client.presigned_get_object.return_value = (
            "https://minio.example.com/test-bucket/test-object"
        )
